...
Eitri App Shopping apps comes with seemless integration with [Firebase Cloud Messaging](https://firebase.google.com/docs/cloud-messaging){:target="_blank"} but it's possible to fire push-notifications through other service providers.
...
Partners that need to send push-notifications using Eitri's infrastructure should use the [Partner Push API](partner-push-api.md), authenticating with a `client_id` and `client_secret` generated by the internal Eitri team.
