...
This page is for **Eitri partners** that need to send push notifications to the devices of an Eitri Shopping store using Eitri's infrastructure.
...
**Credentials are not self-service.** There is no public sign-up: they are generated by the Eitri team upon request.
...
!!! warning "The `client_secret` is handed over only once"
    The secret cannot be retrieved later. Keep it in a secret manager (Vault, AWS Secrets Manager, etc.). If it is lost or leaked, ask the Eitri team for a **rotation**: a new secret is generated and the previous one stops working immediately.

### Credential lifecycle

All the operations below are performed by the Eitri team, upon request:

- **Creation** — generates the `client_id` and `client_secret` for a store.
...
- **Revocation** — deactivates the credential. Already-issued tokens stop working immediately.
...
Delivery is **asynchronous**: the `202` means the request was accepted, not that the notification already reached the device. Keep the `requestId` — it is what you give Eitri support to trace a delivery.

Unknown devices, or devices that do not belong to your credential's store, are ignored during processing. So `enqueued` reflects what was accepted, not what was actually delivered.
